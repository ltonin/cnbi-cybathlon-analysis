...
for sId = 1:NumSubjects
   ...
   for dmId = 1:length(cmonths)
        ...
        cindex2 = Sk == sId & Dml == cmonths(dmId) & Cyk == false;
        cnruns = sum(cindex);
        ...
        %imagesc(freqs(1:15), 1:16, cdata(1:15, :)', maplimits);
         imagesc(freqs, 1:16, cdata(:, :)', maplimits);
        axis image;
    ...
    % Competition day
    subplot(NumRows, NumCols, NumCols + NumCols*(sId -1) - 1);
...
cindex2 = Sk == 2 & Dml == 10 & Cyk == false;
crunid = find(cindex2);
for i = 1:length(crunid)

   subplot(5, 4, i); 

    cdata = nanmean(fisherscore(:, :, crunid(i)), 3);
...
excludedId = 274:291;
cindex3 = cindex2;
for r = 1:length(excludedId)
    cindex3(excludedId(r)) = false;
end
cdata = nanmean(fisherscore(:, :, cindex3), 3);
